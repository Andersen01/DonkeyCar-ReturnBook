...
def split_train_val_sets(images_dir, val_ratio=0.02):
    ...
    image_files = glob.glob(os.path.join(images_dir, '*.jpg'))#返回所有jpg的列表
    ...
    train_dict = _get_labling_dict(train_files)#调用_get_labling_dict()函数
    val_dict = _get_labling_dict(val_files)
    return train_dict, val_dict


def _get_labling_dict(image_files=None):#获得标签地点
    ...
    for image_file in image_files:
        ...
        if image_name.startswith('one'):
            labling_dict[image_file] = 0
        elif image_name.startswith('two'):
            labling_dict[image_file] = 1
    return labling_dict


def write_annotation_json(images_dir, train_json_output_path,  #写json文件，使用image_dir
                          val_json_output_path):
    """Save training and validation annotations."""
    train_files_dict, val_files_dict = split_train_val_sets(images_dir)#调用split_train_val_sets()函数，从图片中读数据
    train_json = json.dumps(train_files_dict)  #dumps()转为json,loads()转为dict
    ...
    val_json = json.dumps(val_files_dict)#dumps()转为json,loads()转为dict
